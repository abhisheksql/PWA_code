...
function Forgotpassword() {
  ...
  useEffect(() => {
    if (submit) {
      ...
      //   // Return true if there is no "@" symbol in the string, otherwise false
      //   return !input.includes("@");
      // }
      // Example usage
      // const isUser = isUsername(username);
      // if (isUser) {
        let apiUrl;
        let userdata;
        let userdatanew;
        ...
        userdatanew = { credentials: username, role: userrole };
        userdata = { username: username };
        const apiCall = async () => {
          try {
            const response = await axios.post(apiUrl, userdatanew, {
              headers: {
            ...
            if (response.status === 200 && response.data.valid) {
              ...
              const requestresponse = await axios.post(
                `${leapApiUrl}onboarding/request-reset-password/`,
                userdata,
                {
                  headers: {
                    "Content-Type": "application/json",
              ...
              if (requestresponse.status === 200) {
                router.push(
                  `/login/teacher/mobileotp?data=${username}&action=forgotpassword&mobile=${requestresponse.data.data.phone_number}&email=${requestresponse.data.data.email}`
                );
              } else {
                console.error(
              ...
              <div className="login-box mobileotp-box">
                ...
                <form onSubmit={loginhandler}>
                  ...
                  >
                    ...
                    {error && <span>Invalid username or not registered</span>}
                  </div>
